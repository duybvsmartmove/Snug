...
//
// Chỉ một file duy nhất được ghi: public/content/levels.json. Commit xong, workflow Pages
...
export const REPO = {
  ...
  path: 'public/content/levels.json',
};
...
/** Ghi levels.json thành một commit */
export async function putBook(book, note = '') {
  const token = readToken();
  if (!token) throw new Error('chưa có token');
  const url = `/repos/${REPO.owner}/${REPO.repo}/contents/${encodeURI(REPO.path)}`;
  ...
  const r = await call(url, {
    method: 'PUT',
    body: JSON.stringify({
      message: `content: sắp xếp level v${book.version}${note ? ' — ' + note : ''}`,
      content: toBase64(JSON.stringify(book, null, 2)),
      branch: REPO.branch,
      ...(sha ? { sha } : {}),
    }),
  }, token);
